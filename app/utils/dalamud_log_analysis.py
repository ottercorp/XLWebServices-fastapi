...
def merge_message(df, index, start_index, drop_index_list) -> int:
    ...
    Args:
        ...
        index :int: 当前行的索引
        start_index :int: 需要合并的行的起始索引
        drop_index_list: list: 需要删除的行的索引列表
...
def decode_base64_message(message: str) -> dict:
    """
    解码base64并解析为JSON字典
    Args:
        message (str): base64编码的字符串

    Returns:
        dict: 解码后的JSON字典
    """
    try:
        base64_ciphertext = message.split(':', 1)[1]
...
def analysis(file_object, api_level: int = 6) -> tuple[dict, int | None]:
    """分析日志方法
    ...
    for index, row in df.iterrows():
        try:
            ...
            if 'LASTEXCEPTION' in row['message']:
                # 进行base64解码
                plain_dict = decode_base64_message(row['message'])
                # 合并last_exception字典
    ...
    if log_file_type == 0:
        ...
        for i in LoadedPlugins_list:
            ...
            if i['Disabled'] is True:
                ...
                disabled_plugins_list.append(plugin_name)
                plugin_status = "error_api_level"
            else:
                third_party_plugin_list.append(plugin_name)
                plugin_status = "3rd"
            LoadedPlugins_dict[plugin_name] = {'plugin_status': plugin_status,
